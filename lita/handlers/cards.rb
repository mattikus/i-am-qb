QUESTIONS = %w[
  ...
  In 1000 years when paper money is but a distant memory _____ will be our currency.
...
module Lita
  module Handlers
    class Cards < Handler
      route(/card(?: me)?(?: )(\d+)?/i, :card, command: true, help: {
        "card me" => "Get an answer card",
        "card me 2" => "Displays two answers for questions with two blanks",
      })

      route(/q(?:uestion)? card/i, :question, command: true, help: {
        "q card" => "Returns a question",
      })

      def card(response)
        count = (response.match_data[1] || "1").to_i
        ANSWERS.shuffle[1..count].each { |answer| response.reply(answer) }
      end

      def question(response)
        response.reply(QUESTIONS.shuffle.first)
      end
    end

    Lita.register_handler(Cards)
  end
end
